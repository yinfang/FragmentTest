...
public class ImageCache {
	...
	// /**
	// * Find and return an existing ImageCache stored in a {@link
	// RetainFragment}
	...
	// * not exist
	// */
	// public static ImageCache findOrCreateCache(FragmentManager
	// fragmentManager,
	...
	public void addBitmapToCache(String data, Bitmap bitmap) {
		...
		// marked by chenyh 2014-07-11
		/* 不写入内存，因为对于大图，低端机会发生内存溢出现象 */
	...
	public Bitmap getBitmapFromDiskCache(String data) {
		...
		synchronized (mDiskCacheLock) {
			...
			if (mDiskLruCache != null) {
				...
				try {
					// modified by chenyh 2014-07-25
					// fetch image cache directly since snapshot return null
					// sometimes in old source
					File diskCacheDir = mCacheParams.diskCacheDir;
					File f = new File(diskCacheDir, key + ".0");
					if (f.exists()) {
	...
	public static String hashKeyForDisk(String key) {
		...
		try {
			...
			cacheKey = U.bytes2HexString(mDigest.digest());
	...
	// * @return The existing instance of the Fragment or the new instance if
	// just
	// * created.
	// */
	...
	// /**
	// * A simple non-UI Fragment that stores a single Object and is retained
	// over
